 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Trulia_Sniffs_NamingConventions_UnderscoreLowercaseFunctionNameSniff  extends Generic_Sniffs_NamingConventions_CamelCapsFunctionNameSniff
{
        ...
        if (PHP_CodeSniffer::isUnderscoreName(strtoupper($methodName)) === false
          || $methodName !== strtolower($methodName)
            ...
            if ($methodProps['scope_specified'] === true) {
                $error = '%s method name "%s" is not in lowercase underscore format';
                $data  = array(
                          ucfirst($methodProps['scope']),
                          $errorData[0],
                         );
                $phpcsFile->addError($error, $stackPtr, 'ScopeNotLowercaseUnderscore', $data);
            } else {
                $error = 'Method name "%s" is not in camel caps format';
                $phpcsFile->addError($error, $stackPtr, 'NotLowercaseUnderscore', $errorData);
            }
