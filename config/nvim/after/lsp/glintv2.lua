...
return {
  cmd = {
    'node',
    util.path.join('node_modules', '@glint', 'ember-tsc', 'bin', 'glint-language-server.js'),
    '--stdio'
  },
  filetypes = {
    'typescript',
  ...
  on_new_config = function(config)
    if config.root_dir then
      local root = util.path.normalize(config.root_dir)
      config.cmd[2] = util.path.join(root, config.cmd[2])
    end
  end,
    ...
    local function typescriptHandler(_, result, context)
      ...
      if not ts_client then
        ...
        if retries <= 10 then
          ...
          vim.notify(
            'Could not find `ts_ls`, `vtsls`, or `typescript-tools` lsp client required by `glint v2`.',
      ...
      ts_client:exec_cmd({
        title = 'glint_request_forward', -- You can give title anything as it's used to represent a command in the UI, `:h Client:exec_cmd`
